...
import { BookOpen, Video, Info, Gamepad2 } from "lucide-react";
import { motion } from "motion/react";

interface NavigationProps {
  activeTab: "game" | "video" | "tentang";
  setActiveTab: (tab: "game" | "video" | "tentang") => void;
}
...
export default function Navigation({ activeTab, setActiveTab }: NavigationProps) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto px-4 mt-8 sm:mt-12 relative z-20">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              id={`nav-btn-${tab.id}`}
              onClick={() => setActiveTab(tab.id as "game" | "video" | "tentang")}
              className={`relative flex-1 flex items-center justify-center gap-2 py-4 px-4 text-sm sm:text-base font-bold tracking-wide transition-all duration-300 rounded-[2.5rem] cursor-pointer focus:outline-none focus-visible:ring-4 focus-visible:ring-joy-yellow overflow-hidden ${
                isActive
